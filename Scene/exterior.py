...
class Exterior:
    def __init__(self, game_state: GameState):
        ...
        #if player switches back
        self.interior_has_ran = self.game_state.has_interior_ran
        ...
        if self.interior_has_ran == True:
            self.player_health = self.game_state.exterior_player_health
            self.health_info["paths"][self.health_index] = self.game_state.exterior_health_frame
            self.health_index = self.game_state.exterior_health_index
            int(self.remaining_time) == self.game_state.exterior_remaining_time
            self.time_index = self.game_state.exterior_time_frame
            self.current_customer = self.game_state.current_customer
            holes = self.game_state.exterior_holes
        #end of interior setup
        ...
        while True:
            ...
            for e in pg.event.get():
                if e.type == pg.QUIT:
                    ...
                    self.game_state.exterior_holes = holes
            ...
            if self.show_delivery:
                ...
                if self.pizza_spawned:
                    ...
                    if distance > 5:
                        ...
                        # When the pizza is close enough, set it exactly to the target and trigger game over
                        self.pizza_rect.center = self.target_xy
                        self.screen.blit(pg.transform.rotate(self.pizza_img, self.pizza_angle), self.pizza_rect) # Ensure final draw
                        yay_sfx.play()
